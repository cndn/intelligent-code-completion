...
def test(model,corpus,code,suggestion=3):
    ...
    while len(tokens) > 0 and tokens[-1][-1] == '':
        tokens = tokens[:-1]
    print tokens
    for token in tokens:
        word_idx = corpus.dictionary.word2idx.get(token, 0)
        input.data.fill_(word_idx)
        output, hidden = model(input, hidden)
    word_weights = output.squeeze().data.div(args.temperature).exp().cpu()
    word_idx = torch.topk(word_weights, suggestion)[1]
    # top 1 suggestion
    res = corpus.dictionary.idx2word[word_idx[0]]
    ...
    print res
    return [res[1]]
    # return [corpus.dictionary.idx2word[i][1] for i in word_idx]
