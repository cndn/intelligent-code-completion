...
class Test(object):
    def __init__(self):
        ...
        self.indent = 0
        self.corpus = data.Corpus(args.data, '../statistics/tf_count_with_type.txt')
        self.model = PSMM(args.batch_size, len(self.corpus.dict), args.hidden, args.cuda)
    ...
    def predict_next(self, code,suggestion=1):
        ntokens = len(self.corpus.dict)
        hidden = self.model.reset_hidden()
        ...
        for token in tokens[:-1]:
            word_idx = self.corpus.dict.word2idx.get(token, 0)
            input.data.fill_(word_idx)
            result, _ = model(data)
        word_weights = result.squeeze().data.div(args.temperature).exp().cpu()
        word_idx = torch.topk(word_weights, suggestion)[1]
